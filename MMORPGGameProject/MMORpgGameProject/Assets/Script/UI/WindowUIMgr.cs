...
public class WindowUIMgr : Singleton<WindowUIMgr> 
{
    ...
    public  WindowUIType m_currentOpenWindow = WindowUIType.None;
    /// <summary>
    /// 已经打开的窗口数量
    /// </summary>
    ...
    private void ShowCenterToBig(UIWindowViewBase windowBase, bool isOpen)
    {
        ...
        {
            windowBase.transform.DOScale(Vector3.zero, windowBase.duration).SetAutoKill(false)
               .OnComplete(() => DestroyWindow(windowBase));
        }
    }
    ...
    private void ShowFromDir(UIWindowViewBase windowBase, int dirType, bool isOpen)
    {
        ...
        Vector3 UIWindowLocal = windowBase.transform.position;
        windowBase.transform.localPosition = from;
       
        if (isOpen)
        {
            windowBase.gameObject.SetActive(true);
            windowBase.transform.DOMove(UIWindowLocal, windowBase.duration);
        }
